...
void roll_kalman_filter_update(float sensor, float dt, States *s, Matrix *q, Covariances *covs, R_error *r_vals)
{   
    /* sensor is angular velocity */
...
void roll_kalman_filter_predict(float input, float dt, States *s, Matrix *q_t, Matrix *q_w, Covariances *covs, R_error *r_vals)
{

...
void roll_kalman_filter_predict(float input, float dt, States *s, Matrix *q)
{
    ...
    s->x2 = X21 + U2;
    /*
    c matrix = cov11_angle_roll cov12_angle_roll
               cov21_angle_roll cov22_angle_roll
    */

    /* Dot product covariance and state transistion model*/
    //printf("Covs: %0.15f %0.15f %0.15f %0.15f\n", cov11_angle_roll, cov12_angle_roll, cov21_angle_roll, cov22_angle_roll);
    float c11 = cov11_angle_roll + (F12 * cov21_angle_roll);
    float c12 = cov12_angle_roll + (F12 * cov22_angle_roll);
    float c21 = cov21_angle_roll;
    float c22 = cov22_angle_roll;
    //printf("Cs: %0.15f %0.15f %0.15f %0.15f\n", c11, c12, c21, c22);
    /* c matrix transposed with state transistion model
    Fm11 Fm12    Fm11 Fm21
    Fm21 Fm22 => Fm12 Fm22
    */
    ...
    cov11_angle_roll = p11 + q->m11;  
    cov12_angle_roll = p12 + q->m12;
    cov21_angle_roll = p21 + q->m21;
    cov22_angle_roll = p22 + q->m22;
    //printf("Covs: %0.15f %0.15f %0.15f %0.15f\n", cov11_angle_roll, cov12_angle_roll, cov21_angle_roll, cov22_angle_roll);
}
...
cov21_angle_roll = p21 + q->m21;
cov22_angle_roll = p22 + q->m22;
void roll_kalman_filter_update(float sensor, float dt, States *s, Matrix *q_r)
{   
    /* sensor is angular velocity */
    ...
    /* Gain calculation */
    float gain1 = cov12_angle_roll * (1 / (cov22_angle_roll + r_angle_roll));
    float gain2 = cov22_angle_roll * (1 / (cov22_angle_roll + r_angle_roll));
    //printf("Gain1: %f Gain2: %f\n", gain1, gain2);
    /* Update states */
    ...
    /* Covariance update */
    //printf("Update covs: %0.15f %0.15f %0.15f %0.15f\n", cov11_angle_roll, cov12_angle_roll, cov21_angle_roll, cov22_angle_roll);
    float c11 = (-cov12_angle_roll*cov21_angle_roll + cov11_angle_roll*(cov22_angle_roll+r_angle_roll))/(cov22_angle_roll + r_angle_roll);
    float c12 = (cov12_angle_roll*r_angle_roll)/(cov22_angle_roll + r_angle_roll);
    float c21 = (cov21_angle_roll*r_angle_roll)/(cov22_angle_roll + r_angle_roll);
    float c22 = (cov22_angle_roll*r_angle_roll)/(cov22_angle_roll + r_angle_roll);

    cov11_angle_roll = c11;
    cov12_angle_roll = c12;
    cov21_angle_roll = c21;
    cov22_angle_roll = c22;
    //printf("Update after covs: %0.15f %0.15f %0.15f %0.15f\n", cov11_angle_roll, cov12_angle_roll, cov21_angle_roll, cov22_angle_roll);
